...
type Renderable = Record<string, any> | PromiseLike<Record<string, any>> | null;

function isPromise<T>(thing: T | PromiseLike<T>): thing is PromiseLike<T> {
...
export function Render(template: string) {
    return TransformOutput<Renderable, void>(async (out, req, res, next) => {
        try {
            ...
            if (!out) return next(new NotFound());
