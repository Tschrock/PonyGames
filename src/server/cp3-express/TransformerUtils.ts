...
export function buildTransforms<T extends IHaveHandlers>(baseObject: T, handlerKey: keyof T & string) {
    ...
    const parametersTransformers = Reflect.getOwnMetadata(METAKEY_PARAMETER_TRANSFORMS, baseObject, handlerKey) as Map<number, ValueTransformer<any, any, any>[]>;
    const outputTransformers = getMetadataList<ValueTransformer<any, any, any>>(METAKEY_OUTPUT_TRANSFORMS, baseObject, handlerKey);
